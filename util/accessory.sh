...
#! nix-shell -i dash -I channel:nixos-24.11-small -p nix dash findutils ncurses jq yq
. ./prelude
...
if [ "${HOMEKIT_SH_CACHE_TOML_ACCESSORIES:-false}" = "true" ]; then
  jq -cs "{ aid: $aid, services: map({type, typeName, iid, characteristics}) }" "$HOMEKIT_SH_CACHE_DIR/$toml/accessory.json"
  ...
  dash ./util/services_grouped_by_type.sh "$toml" \
    ...
    | jq -cs "{ aid: $aid, services: map({type, typeName, iid, characteristics}) }"
fi
