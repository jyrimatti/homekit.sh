...
fn main() -> Result<(), anyhow::Error> {
    ...
    let authtag = cipher.encrypt_in_place_detached(GenericArray::<u8,U12>::from_slice(&nonce), b"", &mut result).ok().unwrap();
    
    // print concatenated ciphertext and authtag as hex
    print!("{}", hex::encode(result) + &hex::encode(&authtag));
    Ok(())
}
